...
# Since there are 1544 unknown values we can assume that they correspond to the never smoked category since this is the mode of this column.
...
# Based on this heatmap we see that the features age, hypertension, ever_married, and avg_glucose_level have the highest correlation with stroke.
# The other features have similar correlations so we should not drop any.

# Since age and avg_glucose_level are the numerical features that have the strongest correlation with stroke, we need to check what distribution they follow.
...
# We create our sample matrix X by removing the columns id and stroke and create our target vector y by assigning the column stroke to it.
